...
public class SalesController(IMediator mediator) : BaseController
{
    ...
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ApiResponseWithData<GetUserResponse>), StatusCodes.Status200OK)]
    public async Task<IActionResult> UpdateSale([FromRoute] Guid id, [FromBody] ChangeSaleCommand command, CancellationToken cancellationToken)
    {
        command.Id = id;
        var result = await mediator.Send(command, cancellationToken);

        return Created(string.Empty, new ApiResponseWithData<ChangeSaleResult>
        {
            Data = result,
            Success = true,
