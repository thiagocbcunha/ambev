...
public class SaleRepository(DefaultContext context) : ISaleRepository
{
    ...
    /// <inheritdoc />
    [CachePolicy(60)]
    public async Task<List<Sale>> ListByFilterAsync(DateTime? startDate, DateTime? endDate, string? custormerEmail, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrEmpty(custormerEmail))
            query = query.Where(s => s.Customer.Email.Equals(custormerEmail));
